...
さらに\.{middle}\index{\.{middle}}\ primitiveが追加された．
...
\epTeX~110227以降では\node{dir\_node}と\node{disp\_node}は
\.{lastnodetype}\index{\.{lastnodetype}}の対象とする「最後のノード」とはならないようにしている
\footnote{%
...
\.{currentiftype}\index{\.{currentiftype}}における条件判断文とそれを表す数字との対応は，以
下のようになっている．21--27が，\pTeX 拡張で追加された条件判断文に対応する．
28の\.{ifpdfprimitive}は\hologo{pdfTeX}由来のプリミティブ（後述）である．
\begin{multilist}{3}{\hfil\hskip1.5zw#:&\quad#\qquad\hfil}
...
\item \bf 26&\.{ifybox}\cr\item \bf 27&\.{ifdbox}\cr\item \bf 28&\.{ifpdfprimtive}\cr
\end{multilist}
